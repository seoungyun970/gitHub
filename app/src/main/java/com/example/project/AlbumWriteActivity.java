...
import com.example.project.Model.AlbumWrite;
...
public class AlbumWriteActivity extends Activity {
	ProgressDialog mLoagindDialog;
	GridView mGvImageList;
	ImageAdapter mListAdapter;
	ArrayList<AlbumWrite> mThumbImageInfoList;

	@Override
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.write_album);

		mThumbImageInfoList = new ArrayList<AlbumWrite>();
	...
	private long findThumbList() {
		...
		if (imageCursor != null && imageCursor.getCount() > 0) {
			...
			while (imageCursor.moveToNext()) {
				AlbumWrite thumbInfo = new AlbumWrite();
	...
	private void updateUI() {
		mListAdapter = new ImageAdapter(this, R.layout.write_album_item_image, mThumbImageInfoList);
	...
	private class ImageAdapter extends BaseAdapter {
		...
		private ArrayList<AlbumWrite> mArrData;

		public ImageAdapter(Context c, int cellLayout,
							ArrayList<AlbumWrite> thumbImageInfoList) {
			mContext = c;
		...
		public View getView(final int position, View convertView,
			...
			if (((AlbumWrite) mArrData.get(position)).getCheckedState())
	...
	private class DoFindImageList extends AsyncTask<String, Integer, Long> {
		...
		protected void onPreExecute() {
			mLoagindDialog = ProgressDialog.show(AlbumWriteActivity.this, null, "로딩중...", true, true);
			super.onPreExecute();
		}
